...
import { fetchWordPressPlans } from "@/lib/api";
...
export default async function Page() {
  try {
    const wordpressPlans = await fetchWordPressPlans();

    if (wordpressPlans.length === 0) {
      return (
    ...
    return (
      ...
      <section className="hero min-h-[40vh] relative flex flex-col justify-center items-center gap-6 bg-gradient-to-b from-blue-900/20 via-slate-900/20 to-slate-900">
        <h1 className="text-4xl lg:text-6xl font-bold mb-6 bg-gradient-to-r from-cyan-400 via-blue-400 to-cyan-400 bg-clip-text text-transparent font-mono tracking-wide z-1">Hosting WordPress</h1>
        <div className="text-xl text-muted-foreground max-w-2xl mx-auto text-center z-1">
        Hosting especializado con WordPress preinstalado y optimizaciones específicas para máximo rendimiento.
        </div>
      </section>
      <section className="py-20">
        <div className="container mx-auto">
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {(wordpressPlans as WordPressPlanData[]).map((plan: WordPressPlanData, index: number) => (
              <WordPressPlan
                key={plan.id}
                title={plan.title}
