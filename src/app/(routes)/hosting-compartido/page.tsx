...
import { fetchCompartidoPlans } from "@/lib/api";
...
export default async function Page() {
  try {
    const compartidoPlans = await fetchCompartidoPlans();

    if (compartidoPlans.length === 0) {
      return (
        <section className="container mx-auto py-12 px-4 text-center">
          <h1 className="text-4xl font-bold mb-4">Hosting Compartido</h1>
          <p className="text-xl text-gray-600">
            No hay planes Compartido disponibles en este momento.
          </p>
        </section>
      );
    }
    ...
    return (
      ...
      <section className="hero min-h-[40vh] relative flex flex-col justify-center items-center gap-6 bg-gradient-to-b from-blue-900/20 via-slate-900/20 to-slate-900">
        <h1 className="text-4xl lg:text-6xl font-bold mb-6 bg-gradient-to-r from-cyan-400 via-blue-400 to-cyan-400 bg-clip-text text-transparent font-mono tracking-wide z-1">Hosting Compartido</h1>
        <div className="text-xl text-muted-foreground max-w-2xl mx-auto text-center z-1">
        La solución perfecta para sitios web personales, blogs y pequeñas empresas. Hosting confiable, económico y con todas las herramientas que necesitas.
        </div>
      </section>
      <section className="py-20">
        <div className="container mx-auto">
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {(compartidoPlans as CompartidoPlanData[]).map((plan: CompartidoPlanData, index: number) => (
              <CompartidoPlan
                key={plan.id}
                title={plan.title}
